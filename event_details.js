...
const BtnToPurchase = document.querySelector(".book_event");
...
fetch(`http://localhost:5000/events/${eventId}`)
  ...
  .then((eventData) => {
    ...
    BtnToPurchase.addEventListener("click", function () {
      redirectToPurchase(eventId);
    });
  })
...
function updateEventDetails(eventData) {
  ...
  placeForTime.textContent = formatTime(
    eventData.date_of_event_start,
    eventData.date_of_event_end
  );
  placeForDescription.textContent = eventData.description;
  placeForPriceBox.textContent = eventData.price_for_ticket + " тенге";
  placeForDateBox.textContent =
    formatDate(eventData.date_of_event_start) +
    " - " +
    formatDate(eventData.date_of_event_end);
...
function redirectToPurchase(eventId) {
  window.location.href = `purchase.html?event_id=${eventId}`;
}

fetch(`http://localhost:5000/tickets/count/${eventId}`)
  .then((response) => response.json())
  .then((data) => {
    const placeForAllTickets = document.querySelector(
      ".remaining_tickets_fetch"
    );
    placeForAllTickets.textContent = `${data.tickets_count}`;
  });
